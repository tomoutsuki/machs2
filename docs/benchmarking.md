...
The benchmark measures end-to-end API behavior for the FABEO mode in a reproducible local setup.
...
3. `POST /entries/{entry_id}/decrypt-package` (authorization + decrypt path)
...
- New login session is created for each run.
...
- Benchmark output is most useful for architectural overhead comparisons within this MVP, not as production cryptographic performance claims.
